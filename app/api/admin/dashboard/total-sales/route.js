...
async function handleGet(req, res) {
    ...
    if (session) {
        ...
        // Get total sales (orders with valid statuses)
        const totalSales = await Order.countDocuments({ status: { $in: validStatuses }, delete: false });

        // Function to get daily counts for a week
        ...
        const thisWeekTotal = thisWeekData.reduce((a, b) => a + b.count, 0);
        const lastWeekTotal = lastWeekData.reduce((a, b) => a + b.count, 0);
