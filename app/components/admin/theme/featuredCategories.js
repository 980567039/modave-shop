...
const ProductItem = ({ product, isSelected, onAdd }) => (
    ...
    >
        <div className="flex items-center gap-3">
            <img
                src={product?.categoryFeaturedImage?.url || "https://dummyimage.com/400x400/ddd/000"}
                width={60}
                height={70}
...
const SelectedProduct = ({ product, onRemove }) => {
    const allFormattedImages = product;
    return (
        <div className="bg-white p-1">
            <AspectRatio className="relative overflow-hidden" ratio={8 / 12}>
                ...
                <img
                    src={allFormattedImages?.categoryFeaturedImage?.url || "https://dummyimage.com/600x700/ddd/000"}
                    width={400}
                    height={500}
                    alt={allFormattedImages.title || "分类图片"}
...
export default function FeaturedCategories({
    onSelect, isAutoFetch, max = null, findKey = null,
}) {
    ...
    const getSearchedProduct = useCallback(async (query) => {
        ...
        try {
            ...
            setShowResults(true);
            setInputAlert('')
            const url = `admin/categories/filter?query=${query}`;
            ...
            toast.error("获取分类失败", { description: error.message });
            setProducts([]);
        } finally {
            setIsLoading(false);
        }
    ...
    useEffect(() => {
        if (store?.theme && findKey) {
            ...
            setSelectedProducts(themeSelectedProduct || []);
        }
    }, [store, findKey]);
    ...
    return (
        <div className='space-y-3'>
            <h2 className='font-semibold'>选择精选分类</h2>
